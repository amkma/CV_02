 ...
 * Task 2: Active Contours (Snake):
   ...
   * Adjust the energy weights:
     * \alpha (Continuity): Controls the elasticity of the contour.
     * \beta (Curvature): Controls the stiffness/smoothness.
     * \gamma (Image): Controls the magnetic pull towards the object's edges.
...
The active contour minimizes the following integral:
In this discrete implementation:
 * E_{img} is optimized via Gradient Descent against a Chamfer 2-pass distance transform.
 * E_{cont} + E_{curv} is approximated efficiently using Laplacian Smoothing (averaging neighboring points).
Geometric Feature Extraction
Area is calculated rigorously using the Shoelace Formula over the final converged polynomial points:
File Descriptions
